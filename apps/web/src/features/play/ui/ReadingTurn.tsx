...
export function ReadingTurn({
  ...
  if (message.role === "user") {
    return (
      <div
        id={`turn-${message.id}`}
        className={cn(
          "w-full rounded-xl px-1 py-1 transition-colors",
          highlighted && "message-highlight",
        )}
      >
        <div className="flex items-baseline justify-end gap-3">
          <span className="text-[10px] font-medium uppercase tracking-[0.14em] text-fg-faint">
            {COPY.play.you}
          </span>
        </div>
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.14em] text-fg-faint">
