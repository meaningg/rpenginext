...
/** Registry reference of the built-in fallback profile (ADR 0007 D2). */
export const BUILTIN_DEFAULT_PROFILE_REF = "default@1.0.0";

/**
 * Returns the built-in default narrative prompt profile.
 *
 * Always available (tests / CI / host without prompt files); cannot be
 * overridden by a file with the same `id@version` (registry fails instead).
 */
export function getBuiltinDefaultProfile(): NarrativePromptProfile {
  return BUILTIN_DEFAULT_PROFILE;
}

/**
 * Built-in narrative prompt profile `default@1.0.0` (ADR 0007 D2).
 *
 * Text is the exact narrative prompt shipped at Module Platform 1.0
 * (previously hardcoded in `narrative-write.ts`), re-expressed as data with
 * placeholders. Any change here changes the default prompt — keep it aligned
 * with golden turn tests.
 */
export const BUILTIN_DEFAULT_PROFILE = Object.freeze({
  id: "default",
  version: "1.0.0",
  description:
    "Core built-in narrative prompt (Platform 1.0 text, RU) — always available fallback",
  labels: {
  ...
  systemCore: [
    ...
    "",
    "Формат сообщений:",
    "- Последнее user-сообщение содержит «{{playerActionLabel}} …» — это текущий поступок персонажа в мире, который нужно разрешить сейчас. Это не реплика в диалоге с тобой.",
...
} satisfies NarrativePromptProfile);
