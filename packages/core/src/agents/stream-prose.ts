...
/**
 * Returns only the newly available prose suffix after appending a raw delta.
 *
 * @param previousRaw - buffer before this delta
 * @param delta - raw model delta (may be JSON fragment)
 */
export function narrativeProseDelta(
  previousRaw: string,
  delta: string,
): { readonly nextRaw: string; readonly proseDelta: string } {
  const nextRaw = previousRaw + delta;
  const prevProse = extractStreamingProse(previousRaw);
  const nextProse = extractStreamingProse(nextRaw);
  if (!nextProse.startsWith(prevProse)) {
    // Model rewrote earlier content; resend full prose as a single delta.
    return { nextRaw, proseDelta: nextProse };
  }
  return {
    nextRaw,
    proseDelta: nextProse.slice(prevProse.length),
  };
}
